...
(function ()
{
   ...
   Bee.Dom = {
      ...
      {
         if (Bu.defined(element.remove))
         {
            element.remove();
         }
         else if (Bu.defined(element.parentNode) && element.parentNode.hasChildNodes())
         {
            element.parentNode.removeChild(element);
         }
         else
         {
            throw new Error(element + " cannot be removed");
         }
      },
       ...
       */
      setDisplay      : function (el, newDisplayState)
      {
         ...
         {//console.log(fade);
            ...
            else
            {
               ...
               else
               {
                  if (!delay)
                  {
                     this.setDisplay(windowEl, "none");
                  }
                  else
                  {
                     var clockOut2 = setTimeout(function ()
                                                {
                                                   Bee.Utils.setDisplay(windowEl, "none");
               ...
               var clockOut = setTimeout(function ()
                                         {
                                            ...
                                            Bee.Utils.setDisplay(windowEl, "block");
                                         }, delayVal);
            }
            else
            {
               this.setDisplay(windowEl, "block");
            }
         }
      },
      /**
      ...
      toggleDisplay  : function (windowEl, delay)
      {
         ...
         if (Bs.isEmpty(windowEl.style.display) || self.getDisplayState(windowEl) === 0)
         {
            ...
            else
            {
               var clockOut = setTimeout(function ()
                                         {
                                            self.setDisplay(windowEl, "block");
                                            clearTimeout(clockOut);
                                         }, delayVal);
            }
         }
       ...
       *@param el {Element}
       *@param el2 {Element}*/
      swapElsDisplay : function (el, el2)
      {
         if (this.getDisplayState(el) === 0)
         {
            this.setDisplay(el2, "none");
            this.setDisplay(el, "block");
            return false;
         }
         this.setDisplay(el, "none");
         this.setDisplay(el2, "block");
         //this.toggleClass(el2, newAnim, currentAnim);
         //this.toggleClass(el, newAnim, currentAnim);
      },

      /**@use: the following function activates(distinguishes)
       * a new object that is a part of a list of nodes
